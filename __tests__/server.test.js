...
describe('basic Auth',()=>{
    ...
    it('signin',async ()=>{
        const response = await request.post('/signin').auth("walid","1234");
        expect(response.status).toBe(200);
    });
    //403 status for wrong input
    it('sign in with wrong password',async ()=>{
        const response = await  request.post('/signin').auth('walid','gsfgsf');
        expect(response.status).toBe(403)
    })
    it('sign in with wrong username',async ()=>{
        const response = await  request.post('/signin').auth('walidee','1234');
        expect(response.status).toBe(403)
    })
})